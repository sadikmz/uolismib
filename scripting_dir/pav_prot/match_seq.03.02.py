...
"""
Compare reference protein (ref_prot) sequences vs query protein sequences pairs from full_dict. It return number of matching, deleted, inserted and substituted amino acids and append the result to full_dict dictionary.

Run diamond blastp for each pair of sequences and parse the output to get the alignment information.

This will be a new called class called CompareProt that run diamond blastp and parse the output to get the alignment information and append it o full_dict dictionary. use diamond output headers as keys (eg. qseqid) and its value as values.

Here is bash script to run diamond blastp for each pair of sequences

cpus=48
diamond makedb --in ref_prot.faa -d ref_prot.faa
...
--ultra-sensitive \
--masking none \
--out diamond.${qry_name}_${ref_name}.vsensitive.e06.tsv \
--outfmt '6 qseqid qlen sallseqid slen qstart qend sstart send evalue bitscore corrected_bitscore score length pident nident mismatch gapopen gaps qcovhsp scovhsp qstrand' \
--compress 1 \
--unal 1 \
--evalue 1e-6 \
--threads $cpus \
...
ref_prot.faa example header:
>XP_031041156.3 uncharacterized protein FOBCDRAFT_266382 [Fusarium oxysporum Fo47]

qry_prot.faa example header:
>FOZG_00001-t36_1-p1 | transcript=FOZG_00001-t36_1 | gene=FOZG_00001 | organism=Fusarium_oxysporum_Fo47 | gene_product=hypothetical protein | transcript_product=hypothetical protein | location=JH717896.1:3086-3886(+) | protein_length=266 | sequence_SO=supercontig | SO=protein_coding_gene | is_pseudo=false
