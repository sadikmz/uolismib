...
PAVprot
Features:
- FASTA parsing with smart header handling for ref and qry
- gffcompare .tracking parsing (grouped by reference gene with optional GFF3 mapping)
- Optional class_code filtering with comma syntax (--class-code em,j)
- Optional DIAMOND protein alignment (--run-diamond + --diamond-threads N)
- Outputs TSV to stdout (or auto-saved file when filtering)
- DIAMOND enrichment adds alignment stats to the dict
"""
...
class PAVprot:
    ...
    def fasta2dict(source, is_query=False):
        """
        Parse FASTA headers correctly based on source type.
        - Reference: >XP_031041156.3 ... → XP_031041156.3
        - Query: >FOZG_00001-t36_1-p1 | ... → FOZG_00001-t36_1 (removes -p + one char)
        """
        if str(source) == '-':
            lines = sys.stdin
            close_at_end = False
        ...
        with context:
            ...
            for line in context:
                ...
                if line.startswith('>'):
                    ...
                    if is_query:
                        # Remove -p + exactly one character at the end
                        if len(transcript_id) >= 3 and transcript_id[-3] == '-' and transcript_id[-2] == 'p' and len(transcript_id[-1]) == 1:
    ...
    def load_feature_table(gff_path: str):
        """Parse GFF3 CDS lines for XM→XP and locus_tag→gene mapping"""
        rna_to_protein = {}
        locus_to_gene = {}
        ...
        with open(gff_path) as f:
            for line in f:
                ...
                if locus_tag:
                    ...
                    # Also map without prefix for fallback
                    short_tag = locus_tag.replace('FOBCDRAFT_', '')
                    locus_to_gene[short_tag] = gene_id
    ...
    def parse_gffcompare_tracking(filepath: str, feature_table: str = None, filter_codes: set = None):
        """Parse .tracking with optional feature table mapping"""
        rna_to_protein = {}
        locus_to_gene = {}
        if feature_table:
...
# =================================================================
class CompareProt:
    def __init__(self, diamond_path: str = "diamond", threads: int = 40, prefix: str = "gffcompare"):
    ...
    def _run_diamond(self, db_faa: str, qry_faa: str, prefix: str=None) -> str:
        out = os.path.join(os.getcwd(), f"{prefix}.tsv.gz")
        cmd = [
            self.diamond, "blastp",
...
def main():
    parser = argparse.ArgumentParser(description="PAVprot – complete pipeline")
    parser.add_argument('--gff-comp', required=True, help="gffcompare .tracking file")
    parser.add_argument('--feature-table', help="GFF3 CDS feature table (optional)")
    ...
    parser.add_argument('--run-diamond', action='store_true', help="Run DIAMOND")
    parser.add_argument('--diamond-threads', type=int, default=40, help="DIAMOND threads")
    parser.add_argument('--output-prefix', default="gffcompare", help="Output file prefix")

    args = parser.parse_args()

    filter_set = {c.strip() for c in args.class_code.split(',')} if args.class_code else None
    full, filtered = PAVprot.parse_gffcompare_tracking(args.gff_comp, args.feature_table, filter_set)
    ...
    if args.run_diamond:
        ...
        comparer = CompareProt(threads=args.diamond_threads, prefix=args.output_prefix)
    ...
    if args.class_code:
        ...
        output_file = f"{args.output_prefix}_{safe_codes}{suffix}"
    else:
        suffix = "_diamond_blastp.tsv" if args.run_diamond else ".tsv"
        output_file = f"{args.output_prefix}{suffix}"
