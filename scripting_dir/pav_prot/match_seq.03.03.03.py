#!/usr/bin/env python3
"""
PAVprot – Complete pipeline (November 20, 2025)
Features:
- FASTA parsing with smart header handling (ref: XP_..., qry: FOZG_...-pX removed)
- gffcompare .tracking parsing grouped by reference gene
- Optional GFF3 feature table mapping (XM_... → XP_..., FOBCDRAFT_... → gene-FOBCDRAFT_...)
- Optional class_code filtering (--class-code em,j)
- Optional DIAMOND protein alignment (--run-diamond + --diamond-threads)
- TSV output to stdout or auto-saved file when filtering
"""
...
# =================================================================
# CLASS 1: PAVprot – Core parsing
# =================================================================
class PAVprot:
    @staticmethod
    def fasta2dict(source, is_query=False):
        """Parse FASTA headers correctly for ref and qry proteins"""
        if str(source) == '-':
            lines = sys.stdin
            close_at_end = False
        else:
        ...
        with context:
            ...
            for line in context:
                ...
                if line.startswith('>'):
                    ...
                    if is_query:
                        # Remove -p + one character (e.g. -p1, -pA)
    ...
    def load_feature_table(gff_path: str):
        """Extract XM_... → XP_... and locus_tag → gene-... mappings from GFF3 CDS lines"""
        rna_to_protein = {}
        locus_to_gene = {}
...
# =================================================================
# CLASS 2: CompareProt – DIAMOND alignment
# =================================================================
class CompareProt:
        ...
        with gzip.open(tsv_gz, 'rt') as f:
            for line in f:
                if not line.strip():
                    continue
                f = line.strip().split('\t')
                hit = self._make_hit_dict(f)
                qid = hit["qseqid"]
                if qid not in hits or hit["bitscore"] > hits[qid]["bitscore"]:
                    hits[qid] = hit
        return hits

    def _make_hit_dict(self, fields):
        return {
            "qseqid": fields[0], "qlen": int(fields[1]), "sallseqid": fields[2], "slen": int(fields[3]),
            "qstart": int(fields[4]), "qend": int(fields[5]), "sstart": int(fields[6]), "send": int(fields[7]),
            "evalue": float(fields[8]), "bitscore": float(fields[9]), "score": float(fields[10]),
            "length": int(fields[11]), "pident": float(fields[12]), "nident": int(fields[13]),
            "mismatch": int(fields[14]), "gapopen": int(fields[15]), "gaps": int(fields[16]),
            "qcovhsp": float(fields[17]), "scovhsp": float(fields[18]), "qstrand": fields[19]
        }
...
# =================================================================
# main() – full pipeline with correct --help
# =================================================================
def main():
    ...
    parser.add_argument('--run-diamond', action='store_true', help="Run DIAMOND protein alignment")
    parser.add_argument('--diamond-threads', type=int, default=48, help="DIAMOND threads (default: 48)")
    ...
    if output_file:
        with open(output_file, 'w') as f:
            ...
            for _, entries in data.items():
                for e in entries:
                    ...
                    if args.run_diamond:
                        f.write(f"{base}\t{e.get('identical_aa', 0)}\t{e.get('mismatched_aa', 0)}\t{e.get('indels_aa', 0)}\t{e.get('aligned_aa', 0)}\n")
                    else:
                        f.write(base + "\n")
        ...
        for _, entries in data.items():
            for e in entries:
                ...
                base = f"{e['ref_gene']}\t{e['ref_transcript']}\t{e['query_gene']}\t{e['query_transcript']}\t{e['class_code']}\t{exons}"
                if args.run_diamond:
                    print(f"{base}\t{e.get('identical_aa', 0)}\t{e.get('mismatched_aa', 0)}\t{e.get('indels_aa', 0)}\t{e.get('aligned_aa', 0)}")
                else:
                    print(base)
