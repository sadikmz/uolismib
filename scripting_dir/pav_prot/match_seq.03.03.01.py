#!/usr/bin/env python3
"""
PAVprot pipeline – Complete and final version (November 20, 2025)
Features:
...
class PAVprot:
    ...
    def fasta2dict(source, is_query=False):
        ...
        - Query: >FOZG_00001-t36_1-p1 | ... → FOZG_00001-t36_1-p1
        """
        if str(source) == '-':
            lines = sys.stdin
            close_at_end = False
        ...
        with context:
            ...
            for line in context:
                ...
                if line.startswith('>'):
                    ...
                    if is_query:
                        # Query: >FOZG_00001-t36_1-p1 | ...
                        transcript_id = header.split()[0].split('|')[0]
                    else:
                        # Reference: >XP_031041156.3 ...
                        transcript_id = header.split()[0]
                        
                        # maually check for -p1, -p2, -p3 suffixes and all of the sequences header trancrips ids contian -p1
                        if transcript_id.endswith('-p1') or transcript_id.endswith('-p2') or transcript_id.endswith('-p3'):
...
def main():
    ...
    parser.add_argument('--diamond-threads', type=int, default=48, help="DIAMOND threads")
