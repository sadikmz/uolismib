...
# =================================================================
class CompareProt:
    def __init__(self, diamond_path: str = "diamond", threads: int = 40):
...
def main():
    parser = argparse.ArgumentParser(
        description="PAVprot – complete pipeline with optional GFF3 mapping"
    )
    ...
    parser.add_argument('--run-diamond', action='store_true', help="Run DIAMOND (opt-in)")
    parser.add_argument('--diamond-threads', type=int, default=40, help="DIAMOND threads (default: 40)")

    args = parser.parse_args()

    filter_set = {c.strip() for c in args.class_code.split(',')} if args.class_code else None

    full, filtered = PAVprot.parse_gffcompare_tracking(
        args.gff_comp,
