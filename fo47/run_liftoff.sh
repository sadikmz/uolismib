...
cat "$genotype"_liftover.gff3_polished | grep -v '#' | cut -f9 | tr ';' "\n" | grep 'Parent' | grep '\.t' | sed 's/Parent=//g' | sort | uniq > "$genotype"_liftover.gff3_polished_ids.txt
...
grep '>' "$ref_prot" | sed 's/>//g' > "$genotype"_v5.6_annotation_transcript_ID_list.txt

# subset liftover annotation transcript IDs list from reference annoation 
grep -f "$genotype"_liftover.gff3_polished_ids.txt.01.txt "$genotype"_v5.6_annotation_transcript_ID_list.txt > "$genotype"_liftover.gff3_polished_ids.txt.02.txt
